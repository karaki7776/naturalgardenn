...
if (isset($_POST['register'])) {
    ...
    $role = 'customer'; // تعيين الدور بشكل افتراضي إلى "customer"
...
if (isset($_POST['login'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($password, $row['Password'])) {
            // تخزين بيانات الجلسة
            $_SESSION['CustomerID'] = $row['CustomerID']; // تعيين معرف المستخدم في الجلسة
            $_SESSION['username'] = $row['Username'];
            $_SESSION['role'] = $row['Role']; // تخزين الدور في الجلسة
            
            // التوجيه بناءً على الدور
            if ($_SESSION['role'] == 'admin') {
                $admin=true;
                header('location:homepage.php'); // صفحة المدير
            } else {
                $admin=false;
                header('location:homepage.php'); // صفحة المستخدم
            }
            exit();
        } else {
